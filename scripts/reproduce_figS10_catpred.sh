...
run_training() {
    ...
    for seed in {0..40..10}; do
      ...
      records_file="$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_pdbrecords.json"
      ...
      python "$TRAINING_SCRIPT" \
        ...
        --data_path "$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_train.csv" \
        --dataset_type regression \
        --separate_test_path "$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_test.csv" \
        --separate_val_path "$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_val.csv" \
        ...
        --save_dir "$CKPT_DIR/kcat_dlkcat_retrain/seed${seed}" --epochs 30 \
  ...
  local retrain="$1"
    ...
    for seed in {0..40..10}; do
      ...
      python "$PREDICTION_SCRIPT" \
        --protein_records_path "$records_file" \
        --test_path "$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_test.csv" \
        --smiles_columns "$smiles_col" \
        --preds_path "$OUTPUT_DIR/kcat_dlkcat/kcat_dlkcat_preds_DLKcatDatasetModel_seed${seed}.csv" \
        --checkpoint_dir "$CKPT_DIR/kcat_dlkcat${retrain}/seed${seed}" \
...
run_analysis() {
    ...
    python "$ANALYSIS_SCRIPT" \
        kcat \
        "$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_train.csv" \
        "$DATA_DIR/external_data/Kcat_combination_0918_wildtype_mutant_test.csv" \
        "$OUTPUT_DIR/kcat/kcat_DLKcatDB_CatPred_results.csv" \
        $OUTPUT_DIR/kcat_dlkcat/kcat_dlkcat_preds_DLKcatDatasetModel_seed* \
        > "$LOG_DIR/kcat_dlkcat_analysis_trainvalModel.log" 2>&1
    echo "   Analysis completed. Results saved to $OUTPUT_DIR/kcat/kcat_DLKcatDB_CatPred_results.csv"
