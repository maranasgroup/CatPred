...
case $1 in
  training)
    ...
    python ./external/UniKP/train_UniKP_DLKcat.py > $LOG_DIR/UniKP_DLKcat_kcat_training.log

    ;;
  analysis)
    echo "Running pipeline: Analysis"
    # Analyze results for each parameter and save outputs to the results directory
    python ./external/UniKP/parse_logs.py $LOG_DIR/UniKP_DLKcat_kcat_training.log "$OUTPUT_DIR/kcat/kcat_DLKcatDB_UniKP_results.csv"
    echo "Saved results to: $OUTPUT_DIR/kcat/kcat_DLKcatDB_UniKP_results.csv"
    ;;
  *)
    usage
    ;;
